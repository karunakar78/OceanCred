...
def send_email_mock(to_email: str, subject: str, body: str, attachment_path: str = None):
    print("=" * 60)
...
def generate_invoice_pdf(transaction_id: int, winner_name: str, final_price: float, credit_key: str, date_str: str) -> str:
    pdf = FPDF()
    pdf.add_page()
    
    # Needs a font
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(200, 10, txt="OceanCred ESG Invoice", ln=True, align='C')
    pdf.ln(10)
    ...
    file_path = os.path.join(os.path.dirname(__file__), "invoices", f"invoice_{transaction_id}.pdf")
