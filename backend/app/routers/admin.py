...
def get_dashboard_stats(
    ...
    current_admin: models.User = Depends(dependencies.require_admin),
):
    total_companies = (
        db.query(models.User).filter(models.User.role == "company").count()
    )
    total_fishermen = (
        db.query(models.User).filter(models.User.role == "fisherman").count()
    )

    total_credits = db.query(func.sum(models.Credit.weight)).scalar() or 0
    total_transaction_value = (
        db.query(func.sum(models.Transaction.final_price))
        .filter(models.Transaction.status == True)
        .scalar()
        or 0
    )
    ...
    active_auctions = (
        db.query(models.Listing).filter(models.Listing.status == "active").count()
    )
...
def simulate_new_auction(
    ...
    try:
        ...
        fisherman = (
            db.query(models.User).filter(models.User.role == "fisherman").first()
        )
        if not fisherman:
            fisherman = models.User(
                email=f"fisher_{random.randint(10, 99)}@seacred.com",
                hashed_password="pw",
                role="fisherman",
                name="Simulated Fisher",
            )
            db.add(fisherman)
            db.commit()
        ...
        return {
            "status": "success",
            "message": f"Simulated Listing #{listing.id} generated successfully.",
        }
