...
def place_bid(listing_id: int, bid: schemas.BidCreate, db: Session = Depends(get_db), current_company: models.User = Depends(dependencies.require_company)):
    listing = db.query(models.Listing).filter(models.Listing.id == listing_id).first()
    if not listing or listing.status != "active":
        raise HTTPException(status_code=400, detail="Listing is not active or doesn't exist.")
    
    if datetime.utcnow() > listing.closes_at:
        raise HTTPException(status_code=400, detail="Auction has already closed.")
        
    highest_bid = db.query(models.Bid).filter(models.Bid.listing_id == listing_id).order_by(models.Bid.amount.desc()).first()
    ...
    if bid.amount <= current_highest:
        raise HTTPException(status_code=400, detail=f"Bid must be higher than current highest bid or min price: {current_highest:.2f}")

    if current_company.wallet_balance < bid.amount:
        raise HTTPException(status_code=400, detail="Insufficient wallet balance to place this bid.")

    new_bid = models.Bid(listing_id=listing_id, company_id=current_company.id, amount=bid.amount)
    db.add(new_bid)
    db.commit()
    db.refresh(new_bid)
...
def close_auction(listing_id: int, db: Session = Depends(get_db)):
    ...
    if listing.status != "active":
        raise HTTPException(status_code=400, detail="Listing is already closed or cancelled")
    ...
    winner = db.query(models.User).filter(models.User.id == highest_bid.company_id).first()
    ...
    if winner.wallet_balance < highest_bid.amount:
        ...
        db.commit()
        return {"status": "failed", "message": "Company doesn't have sufficient balance to pay."}
    ...
    import uuid
    credit_key = str(uuid.uuid4()).upper()[:12] # e.g. "9F4E-2B1A-..." but without dashes
    ...
    for l_id in loser_ids:
        ...
        if loser and loser.email_notifications_enabled:
            ...
            send_email_mock(email_to, f"Auction Closed: Outbid on Listing #{listing.id}", body)
...
def verify_credit(credit_key: str, db: Session = Depends(get_db)):
    cert = db.query(models.Certificate).filter(models.Certificate.credit_key == credit_key).first()
    if not cert:
        raise HTTPException(status_code=404, detail="Invalid Credit Key. No certificate found.")
        
    credit = db.query(models.Credit).filter(models.Credit.unique_key == credit_key).first()
    if not credit:
    ...
    fisherman = db.query(models.User).filter(models.User.id == credit.fisherman_id).first()
