...
def send_otp(req: OTPRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        user = models.User(
            email=f"temp_{req.phone}@oceancred.com",
            hashed_password="N/A",
            role="fisherman",
            phone=req.phone,
...
def verify_otp(req: OTPVerify, db: Session = Depends(get_db)):
    ...
    if not user or user.otp_secret != req.otp:
        return JSONResponse(status_code=401, content={"error": {"code": "INVALID_OTP", "message": "Wrong or expired OTP", "status": 401}})
...
def get_profile(db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    total_credits = db.query(func.sum(models.Credit.credit_amount)).filter(models.Credit.fisherman_id == current_user.id).scalar() or 0
    total_waste_kg = db.query(func.sum(models.Credit.weight)).filter(models.Credit.fisherman_id == current_user.id).scalar() or 0.0
    
    lifetime_earnings = db.query(func.sum(models.Transaction.final_price)).join(models.Listing).join(models.Credit).filter(
        models.Credit.fisherman_id == current_user.id, 
        models.Transaction.status == True
    ).scalar() or 0.0

    sold_credits = db.query(func.sum(models.Credit.credit_amount)).filter(models.Credit.fisherman_id == current_user.id, models.Credit.status == 'sold').scalar() or 0
...
def update_profile(req: ProfileUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
...
def get_uploads(db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    credits = db.query(models.Credit).filter(models.Credit.fisherman_id == current_user.id).order_by(models.Credit.collection_date.desc()).all()
    res = []
    for c in credits:
        res.append({
...
def get_upload_single(id: str, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    id_num = int(id.replace("upl_", ""))
    c = db.query(models.Credit).filter(models.Credit.id == id_num, models.Credit.fisherman_id == current_user.id).first()
    if not c:
...
def get_wallet(db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    credits_list = db.query(models.Credit).filter(models.Credit.fisherman_id == current_user.id).all()
    ...
    plastic = sum(c.credit_amount for c in credits_list if 'plastic' in c.waste_type.lower())
...
def get_wallet_transactions(db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    txs = []
    credits_list = db.query(models.Credit).filter(models.Credit.fisherman_id == current_user.id).all()
    ...
    sales = db.query(models.Transaction).join(models.Listing).join(models.Credit).filter(
        models.Credit.fisherman_id == current_user.id, 
        models.Transaction.status == True
    ).all()
...
def create_listing(req: ListingCreateMobile, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    available_credits = db.query(models.Credit).filter(
        models.Credit.fisherman_id == current_user.id,
        models.Credit.status == 'verified'
    ).all()
    
    total_avail = sum(c.credit_amount for c in available_credits)
    if total_avail < req.credits:
        return JSONResponse(status_code=400, content={"error": {"code": "INSUFFICIENT_CREDITS", "message": "Listing exceeds available balance", "status": 400}})
    
    if not available_credits:
        raise HTTPException(status_code=400, detail="No verified uploads to list.")
        
    main_credit = available_credits[0]
    main_credit.status = 'listed'
    closes_at = datetime.datetime.utcnow() + datetime.timedelta(hours=req.duration_hours)
    
    new_listing = models.Listing(
        credit_id=main_credit.id,
    ...
    return {
        "listing_id": f"lst_{new_listing.id}",
        "credits": req.credits,
...
def get_my_listings(db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    listings = db.query(models.Listing).join(models.Credit).filter(models.Credit.fisherman_id == current_user.id).all()
    res = []
    for l in listings:
        res.append({
...
def get_listing_details(id: str, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    ...
    if listing.bids:
        ...
        for b in sorted_bids:
            bids.append(BidItem(
                ...
                price_per_credit=round(b.amount / (listing.credit.credit_amount or 1), 2),
                total_inr=b.amount,
...
def cancel_listing(id: str, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    lid = int(id.replace("lst_", ""))
    listing = db.query(models.Listing).filter(models.Listing.id == lid).first()
    if not listing:
        raise HTTPException(status_code=404)
...
def accept_bid(id: str, req: AcceptBidRequest, db: Session = Depends(get_db), current_user: models.User = Depends(dependencies.require_fisherman)):
    lid = int(id.replace("lst_", ""))
    ...
    try:
        ...
        if res.get("status") == "success":
            return {"success": True, "payout_inr": "Check Wallet", "transaction_id": "resolved"}
        else:
            return JSONResponse(status_code=400, content={"error": res})
    except Exception as e:
        return JSONResponse(status_code=400, content={"error": {"code": "ACCEPT_ERROR", "message": str(e), "status": 400}})
